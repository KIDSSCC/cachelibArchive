...
class ProtoSystemManagement(ConfigManagement):
    ...
    def receive_config(self):
        ...
        if not pool_and_size:
            return curr_config, True
        pool_name = list(pool_and_size.keys())
        cache_allocation = list(pool_and_size.values())
        
        # performance
        sub_item_log = ['/home/md/cachelibArchive/log/bin_' + name + '_meta.log' for name in pool_name]
        performance = []
        throughput = []
        hitrates = []
