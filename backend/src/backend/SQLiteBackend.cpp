...
bool SQLiteBackend::create_database() {
    ...
    std::string create_table_sql = "CREATE TABLE " + table_name + " (ycsb_key int PRIMARY KEY";
    for (int i = 1; i <= MAX_FIELDS; i++) {
        create_table_sql += ", field" + std::to_string(i) + " varchar(" + std::to_string(MAX_FIELD_SIZE) + ")";
    }
    create_table_sql += ");";
