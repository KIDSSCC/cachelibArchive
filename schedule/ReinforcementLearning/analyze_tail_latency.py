...
with open(file_path, 'r') as file:
    for line in file:
        match = re.search(r"epoch:(\d+):\s+(\d+\.\d+)", line)
        if match:
            parts = line.strip().split(':')  # 以冒号分割
            epoch = int(match.group(1))
            RL_epochs.append(epoch + RL_phase * 107)
            RL_values.append(float(match.group(2)))
            RL_curr_phase_total_latency += float(match.group(2))
            if epoch == 107:
                RL_phase += 1
...
with open(baseline_filepath, 'r') as file:
    for line in file:
        ...
        if match:
            ...
            baseline_values.append(float(match.group(2)))
            baseline_curr_phase_total_latency += float(match.group(2))
            if epoch == 107:
                baseline_phase += 1
                baseline_avg_value.append(baseline_curr_phase_total_latency / 108)
                baseline_curr_phase_total_latency = 0
        

print(f'baseline_avg_value: {baseline_avg_value}')
print(f'RL_avg_value: {RL_avg_value}')
upgrade_rate = [(baseline_avg_value[i] - RL_avg_value[i]) / baseline_avg_value[i] for i in range(len(RL_avg_value))]
print(f'each epoch upgrade rate {upgrade_rate}')
print(f'total upgrade rate: {sum(upgrade_rate) / len(upgrade_rate)}')
# 绘制图形
...
plt.axvline(x=107, color='g', linestyle='--', linewidth=2)
plt.axvline(x=214, color='g', linestyle='--', linewidth=2)
