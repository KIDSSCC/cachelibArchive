...
TDB *_db_alloc(int namelen)
{
	 ...
	 */
	#ifdef KIDSSCC
		if ( NULL == (db = reinterpret_cast<TDB*>(calloc(1, sizeof(TDB)))) ) {
...
STATUS _db_find_and_lock(TDB *db, const char *key, int is_write) {
	...
	if ( fread(&key_ptr_pos, TDB_PRT_SIZE, 1, db->idx_fp) != 1){
		if (TDB_DEBUG){
			...
			#ifdef KIDSSCC
				printf("_db_find_and_lock(): fread key_ptr_pos fail, idx_fp addr:%ld\n", i_off);
			#else
...
STATUS	_db_store(TDB *db, const char *key, const char *value, int mode){
	...
	#ifdef KIDSSCC
		data_record = (tdb_data_record_t*)malloc(sizeof(struct tdb_data_record_t));
	#else
    ...
    //key record
	#ifdef KIDSSCC
		key_record = (tdb_key_record_t*)malloc(sizeof(struct tdb_key_record_t));
	#else
...
STATUS	_db_delete(TDB *db, const char *key){
	...
	if (KEY_EXIST == ret && db->key_ptr_pos == db->idxoff){
		...
		if (TDB_DEBUG){
			#ifdef KIDSSCC
				char *s = key_next_ptr ? const_cast<char*>("has next ptr") : const_cast<char*>("only");
			#else
...
TDB *tdb_open(const char *path, char *mode) {
	/*kidsscc:
		创建新的TDB对象时，初始化其cachelibClient对象并注册pool
	*/
	TDB *ret = _db_open(path, mode);
	#ifdef KID_CACHELIB
